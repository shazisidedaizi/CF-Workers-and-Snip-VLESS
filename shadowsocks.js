...
let subPath = 'mysub';     // 节点订阅路径,不修改将使用UUID作为订阅路径
let proxyIP = 'ProxyIP.SG.CMLiussss.net';  // proxyIP 格式：ip、域名、ip:port、域名:port等,没填写port，默认使用443
let password = '2d15f375-a93f-4955-85cf-aa3ab2f93485';  // 节点UUID
let SSpath = 'mypath';          // 路径验证，如果为空则使用UUID作为验证路径
...
let cfip = [ // 格式:优选域名:端口#备注名称、优选IP:端口#备注名称、[ipv6优选]:端口#备注名称、优选域名#备注 
    ...
    'cf.090227.xyz#USSJC', 'bestcf.030101.xyz#USSJC','www.visa.cn#JPTKY','cfip.1323123.xyz#JPTKY','cf.0sm.com#GBLON','104.16.0.0#USSEA','104.17.0.0#USSJC',
	'104.19.0.0#USLA','104.26.0.0#USSEA','162.159.0.0#USLA','172.66.0.0#USLA','172.64.78.126#USDEN','172.64.78.164#USORD','172.64.78.185#USATL','172.65.90.31#SG',
            ...
            if (request.headers.get('Upgrade') === 'websocket') {
                ...
                if (url.pathname.toLowerCase() === `/sub/${subPath.toLowerCase()}` || url.pathname.toLowerCase() === `/sub/${subPath.toLowerCase()}/`) {
                    ...
                    const ssLinks = cfip.map(cdnItem => {
                        ...
                        const ssNodeName = nodeName ? `${nodeName}-${host}` : `${host}`;
